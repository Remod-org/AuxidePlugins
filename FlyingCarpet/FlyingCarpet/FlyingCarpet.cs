...
internal class FlyingCarpet : RustScript
{
    ...
    public override void Initialize()
    {
        ...
        Permissions.RegisterPermission(Name, "flyingcarpet.use");
        Permissions.RegisterPermission(Name, "flyingcarpet.vip");
        Permissions.RegisterPermission(Name, "flyingcarpet.admin");
        Permissions.RegisterPermission(Name, "flyingcarpet.unlimited");
    }
    ...
    private bool isAllowed(BasePlayer player, string perm) => Permissions.UserHasPermission(perm, player.UserIDString);
    ...
    private bool HasPermission(ConsoleSystem.Arg arg, string permname)
    {
        ...
        return Permissions.UserHasPermission(permname, pl.UserIDString);
    }
    ...
    public void OnChatCommand(BasePlayer player, string command, string[] args = null)
    {
        ...
        {
            case "fc":
                cmdCarpetBuild(player, command, args);
                break;
            case "fcc":
                cmdCarpetCount(player, command, args);
                break;
            case "fcd":
                cmdCarpetDestroy(player, command, args);
                break;
            case "fcg":
                cmdCarpetGiveChat(player, command, args);
                break;
            case "fchelp":
                cmdCarpetHelp(player, command, args);
                break;
            case "fcnav":
                cmdCarpetNav(player, command, args);
                break;
        }
    }
    ...
    {
        bool vip = false;
        if (!Permissions.UserHasPermission("flyingcarpet.use", player.UserIDString)) { Message(player, "notauthorized"); return; }
        if (args.Length == 1 && args[0] == "guiclose")
        {
            CuiHelper.DestroyUi(player, FCGUM);
            return;
        }
        if (Permissions.UserHasPermission("flyingcarpet.vip", player.UserIDString))
        {
            vip = true;
        }
        if (CarpetLimitReached(player, vip)) { Message(player, "maxcarpets"); return; }
        if (configData.BlockInTunnel && player.transform.position.y < -70)
        {
            Message(player, "notunnel");
            return;
        }
    ...
    private void cmdCarpetGiveChat(BasePlayer player, string command, string[] args)
    {
        if (args.Length == 0)
        {
            Message(player, "giveusage");
            return;
        }
        bool vip = false;
        string pname = args[0];

        if (!Permissions.UserHasPermission("flyingcarpet.admin", player.UserIDString)) { Message(player, "notauthorized"); return; }
        if (pname == null) { Message(player, "noplayer", "NAME_OR_ID"); return; }

        BasePlayer bplayer = BasePlayer.Find(pname);
        if (bplayer == null)
        {
            Message(player, "noplayer", pname);
            return;
        }

        if (Permissions.UserHasPermission("flyingcarpet.vip", bplayer.UserIDString))
        {
            vip = true;
        }
        if (CarpetLimitReached(bplayer, vip)) { Message(player, "maxcarpets"); return; }
        AddCarpet(bplayer, bplayer.transform.position);
        Message(player, "gaveplayer", pname);
    }
    ...
    //        if (arg.Args == null)
    //        {
    //            Utils.DoLog("You need to supply a valid SteamId.");
    //            return;
    //        }
    //    }
    ...
    //    if (pname.Length < 1) { Utils.DoLog("Player name or id cannot be null"); return; }

    //    BasePlayer Bplayer = BasePlayer.Find(pname);
    //    if (Bplayer == null) { Utils.DoLog($"Unable to find player '{pname}'"); return; }

    //    BasePlayer player = Bplayer;
    //    if (player.HasPermission("flyingcarpet.vip")) { vip = true; }
    //    if (CarpetLimitReached(Bplayer, vip))
    //    {
    //        Utils.DoLog($"Player '{pname}' has reached maxcarpets"); return;
    //    }
    //    AddCarpet(Bplayer, Bplayer.transform.position);
    //    Utils.DoLog($"Gave carpet to '{Bplayer.displayName}'");
    //}

    //[Command("fcc"), Permission("flyingcarpet.use")]
    private void cmdCarpetCount(BasePlayer player, string command, string[] args)
    {
        if (!Permissions.UserHasPermission("flyingcarpet.use", player.UserIDString)) { Message(player, "notauthorized"); return; }
        if (!loadplayer.ContainsKey(player.userID))
        {
            Message(player, "nocarpets");
            return;
        }
        string ccount = loadplayer[player.userID].carpetcount.ToString();
        DoLog("CarpetCount: " + ccount);
        Message(player, "currcarpets", ccount);
    }

    //[Command("fcd"), Permission("flyingcarpet.use")]
    private void cmdCarpetDestroy(BasePlayer player, string command, string[] args)
    {
        if (!Permissions.UserHasPermission("flyingcarpet.use", player.UserIDString)) { Message(player, "notauthorized"); return; }
        ...
        if (!Permissions.UserHasPermission("flyingcarpet.admin", player.UserIDString) && target != null)
        {
            if (target == "all")
            {
            ...
            if (players.Count == 0)
            {
                Message(player, "PlayerNotFound", target);
                return;
            }
            if (players.Count > 1)
            {
                Message(player, "MultiplePlayers", target, string.Join(", ", players.Select(p => p.displayName).ToArray()));
                return;
            }
    ...
    private void cmdCarpetHelp(BasePlayer player, string command, string[] args)
    {
        if (!Permissions.UserHasPermission("flyingcarpet.use", player.UserIDString)) { Message(player, "notauthorized"); return; }
        Message(player, "helptext1");
        Message(player, "helptext2");
        Message(player, "helptext3");
        Message(player, "helptext4");
    }

    //[Command("fcnav"), Permission("flyingcarpet.use")]
    private void cmdCarpetNav(BasePlayer player, string command, string[] args)
    {
        ...
        Utils.DoLog($"Flying from {player.transform.position} to {target}");
    ...
    private object OnOvenToggle(BaseOven oven, BasePlayer player)
    {
        ...
        catch
        {
            if (oven.GetComponentInParent<CarpetEntity>() && !configData.AllowLantern)
            {
                ...
                Message(player, "nostartseat"); return rtrn;
            }
            DoLog("No carpet or not mounted.");
            return null;
        }

        if (activecarpet.carpetlock?.IsLocked() == true) { Message(player, "carpetlocked"); return rtrn; }
        ...
        if (!activecarpet.FuelCheck())
        {
            ...
            {
                Message(player, "nofuel");
                Message(player, "landingcarpet");
                activecarpet.engineon = false;
            }
        }
        bool ison = activecarpet.engineon;
        if (ison) { activecarpet.islanding = true; Message(player, "landingcarpet"); return null; }
        if (!ison)
        {
    ...
    private void DoLog(string message, bool ismovement = false)
    {
        ...
        if (configData.debugMovement || configData.debug) Utils.DoLog(message);
    }
    ...
    private void AddCarpet(BasePlayer player, Vector3 location)
    {
        ...
        {
            Message(player, "carpetspawned");
            if (carpet.needfuel)
            {
                Message(player, "carpetfuel");
            }
            else
            {
                // Put them in the chair.  They will still need to unlock it.
                Message(player, "carpetnofuel");
    ...
    private void DestroyLocalCarpet(BasePlayer player)
    {
        ...
        foreach (BaseEntity p in carpetlist)
        {
            ...
            if (foundent != null)
            {
                ...
                Message(player, "carpetdestroyed");
            }
        }
        if (!foundcarpet)
        {
            Message(player, "notfound", configData.MinDistance.ToString());
        }
    }
    ...
    private object CanMountEntity(BasePlayer player, BaseMountable mountable)
    {
        ...
        if (activecarpet != null)
        {
            if (!Utils.IsFriend(player.userID, mountable.OwnerID)) return false;
        }
        return null;
    }
    ...
    private object CanPickupEntity(BasePlayer player, BaseCombatEntity entity)
    {
        ...
        if (myparent == "FlyingCarpet" || entity?.name == "FlyingCarpet")
        {
            if (configData.debug)
            {
                ...
                {
                    Utils.DoLog("CanPickupEntity: player trying to pickup the carpet!");
                }
                else if (myparent == "FlyingCarpet")
                {
                    string entity_name = entity?.LookupPrefab().name;
                    Utils.DoLog($"CanPickupEntity: player trying to remove {entity_name} from a carpet!");
                }
            }
    ...
    private object CanDeployItem(BasePlayer player, Deployer deployer, uint entityId)
    {
        ...
        {
            if (configData.debug) Utils.DoLog($"Player {player.displayName} trying to place an item(lock?) on carpet storage!");
            Message(player, "nolock");
            return true;
        }
    ...
    private class CarpetEntity : MonoBehaviour
    {
        ...
        private BaseEntity SpawnPart(string prefab, BaseEntity entitypart, bool setactive, int eulangx, int eulangy, int eulangz, float locposx, float locposy, float locposz, BaseEntity parent, ulong skinid)
        {
            ...
            {
                Utils.DoLog($"SpawnPart: {prefab}, active:{setactive}, angles:({eulangx}, {eulangy}, {eulangz}), position:({locposx}, {locposy}, {locposz}), parent:{parent.ShortPrefabName} skinid:{skinid}");
            }
