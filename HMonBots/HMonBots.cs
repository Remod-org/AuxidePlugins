...
internal class HMonBots : RustScript
{
    ...
    public void OnChatCommand(BasePlayer player, string command, string[] args = null)
    {
        ...
        {
            case "mb":
                cmdMB(player, command, args);
                break;
        }
    }

    private void cmdMB(BasePlayer player, string command, string[] args)
    {
        if (args.Length > 0)
        {
    ...
    {
        spawnpoints = data.ReadObject<SortedDictionary<string, SpawnProfile>>("spawnpoints");
    ...
    public void OnEntitySpawned(NPCPlayerCorpse corpse)
    {
    ...
    private object CanLootPlayer(HumanNPC target, BasePlayer player)
    {
    ...
    public string GetMonBotName(ulong npcid)
    {
    ...
    public void SetMonBotInfo(ulong npcid, string toset, string data, string rot = null)
    {
    ...
    public void GiveMonBot(HumanNPC bot, string itemname, string loc = "wear", ulong skinid = 0, int count = 1)
    {
    ...
    public void GiveMonBot(ulong npcid, string itemname, string loc = "wear", ulong skinid = 0, int count = 1)
    {
    ...
    public void AddProfile(Vector3 location, string profile, string mongroupname = "", int quantity = 0)
    {
    ...
    public class MonBotPlayer : MonoBehaviour
    {
        ...
        private void FixedUpdate()
        {
            ...
            else if (weapon != null)
            {
                ...
                //player.Brain.states[AIState.Roam].StateEnter(player.Brain, player); // Changed in staging to protected...
                player.Brain.ClientCurrentState = AIState.Roam; // ???????
                //player.Brain.AddState(new BaseAIBrain.BasicAIState(AIState.Roam)).StateEnter(player.Brain, player); // Would be a duplicate, possibly
                //bool success = player.Brain.SwitchToState(AIState.Roam); // Also protected
            }
        }
