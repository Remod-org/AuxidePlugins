...
public class HPermissionsManager : RustScript
{
    ...
    bool HasPermission(string id, string perm) => Permissions.UserHasPermission(id, perm);
    ...
    #region Hooks
    void OnPluginLoaded(RustScript plugin)
    {
    ...
    void OnPluginUnloaded(RustScript plugin)
    {
        if (plugin is HPermissionsManager)
            return;
        Wipe();
    ...
    void PMMainUI(BasePlayer player, bool group, int page)
    {
        ...
        if (group)
        {
            foreach (var grp in Permissions.GetGroups().OrderBy(x => x))
            {
                pos1++;
                quantity++;
                if (pos1 > 0 && pos1 < 21)
