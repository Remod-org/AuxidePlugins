...
public class NoDecay : RustScript
{
    ...
    public object OnEntitySaved(BaseNetworkable entity, BaseNetworkable.SaveInfo saveInfo)
    {
        ...
        DoLog("ONENTITYSAVED checking disableWarning config");
        if (configData.Global.disableWarning)
        {
            DoLog("Checking for permission enabled");
            ...
            try
            {
                ...
                {
                    _DoLog("Null baseNetworkable");
                }
            }
            catch (Exception ex)
            {
                DoLog($"OnEntitySaved exception: {ex}");
            }
        }
        return null;
    }
