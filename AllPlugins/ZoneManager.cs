...
public class ZoneManager : RustScript
{
    #region Fields
    [PluginReference] RustScript Backpacks;//, PopupNotifications, Spawns;
    ...
    private void SendMessage(BasePlayer player, string message, params object[] args)
    {
        if (player)
        {
            ...
            Message(player, $"<color={Configuration.Notifications.Color}>{Configuration.Notifications.Prefix}</color> {message}");
        }
        else Puts(message);
    }
    ...
    private void cmdChatZone(BasePlayer player, string command, string[] args)
    {
        ...
        if (args.Length == 0)
        {
            ...
            Message(player, $"<color={Configuration.Notifications.Color}>Flags:</color> {zone.definition.Flags}");
            ShowZone(player, zoneId);
            return;
        }
    ...
    private void cmdChatZonePlayer(BasePlayer player, string command, string[] args)
    {
        ...
        {
            Message(player, "The specified player is not in any zone");
            return;
        }
    ...
    private void cmdChatZoneEntity(BasePlayer player, string command, string[] args)
    {
        ...
        if (!zonedEntities.TryGetValue(baseEntity.net.ID, out entityZones))
        {
            Message(player, "The specified entity is not in any zone");
            return;
        }
    ...
    private void ccmdZone(ConsoleSystem.Arg arg)
    {
        ...
        {
            Message(arg, "You don't have access to this command");
            return;
        }
        ...
        {
            Message(arg, "Zone ID not found or too few arguments supplied: zone <zoneid> <arg> <value>");
            return;
        }
    ...
    private void OpenFlagEditor(BasePlayer player, string zoneId)
    {
        ...
        if (!zone)
        {
            Message(player, $"Error getting zone object with ID: {zoneId}");
            CuiHelper.DestroyUi(player, ZMUI);
        }
    ...
    protected void LoadDefaultConfig() => Configuration = GetBaseConfig();
